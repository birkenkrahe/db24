...
* Part 2: Creating a Database and Table (10 minutes)
  ...
  - **Task 1**: In your Org-mode file, create a section titled "Creating
    a Database and Table".

  - **Task 2**: Use SQLite commands to check which database sqlite3
    writes to, and if it contains any tables already.

  - **Task 3**: Create a simple table named ~customer~ with an integer
