...
* DONE Week 3 - Relational databases and SQLlite queries
#+ATTR_HTML: :WIDTH 400px: 
[[../img/sql_queries.png]]
...
* Week 4 - Database types, projects, and Selecting Data
#+ATTR_HTML: :WIDTH 400px:
[[../img/databases.png]]
...
** TODO 11 Review questions for "Fundamentals of DBMS" ([[https://youtu.be/o36Z_OqC2ac?si=z9eobmcn7bfeS_lp][video]])
...
5. What's a *logical* data model?
   ...
   - conceptual model: "A list of numbered names." 
...
6. What's in the catalog of a DBMS?
   #+begin_notes
   The catalog contains data definitions (metadata) including schema,
   views, and other model aspects (e.g. for transactions or storage),
   and keeps them consistent with one another (i.e. it's not just a
   document but rather a system application - like an OS kernel).
   #+end_notes
